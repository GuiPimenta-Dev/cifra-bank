...
export default class RegisterUserInfo {
  ...
  async registerUserPersonalInfo(input: registerPersonalInfoDTO): Promise<OutputDTO> {
    const body = {
      ...
      email: input.email,
      individual_id: input.transactionId,
    };
  ...
  async registerUserPhone(transactionId: number, stateCode: number, number: number): Promise<OutputDTO> {
    const body = {
      individual_id: transactionId,
