...
export default class PaymentFacade implements PaymentFacadeInterface {
  ...
  async consultBalance(bank: string, agency: string, originAccount: string, auth: AuthDTO): Promise<OutputDTO> {
    const consultBalance = new ConsultBalance(this.httpClient);
    return await consultBalance.getBalance(bank, agency, originAccount, auth);
  }
